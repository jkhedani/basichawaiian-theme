...
$landingPageID = get_connected_object_ID( $post->ID, 'song_lessons_to_topics' );
$currencyTypeID = get_connected_object_ID( $post->ID, 'song_lessons_to_topics', 'topics_to_modules', 'modules_to_units' );

// get list of all connected vocabulary terms
$songLines = new WP_Query( array(
	'connected_type' => 'song_lines_to_song_lessons',
...
$lessonCardCounter = 0;
$lessonCardCount = $songLines->post_count;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('lesson-container'); ?> data-lesson-id="<?php echo $post->ID; ?>">
	...
	<div class="lesson-content">
		...
		while ( $songLines->have_posts() ) : $songLines->the_post();
			...
			$lessonAssessmentOptions[] = get_field('song_answer');
			$lessonAssessmentOptions[] = get_field('song_assessment_option_one');
			$lessonAssessmentOptions[] = get_field('song_assessment_option_two');
