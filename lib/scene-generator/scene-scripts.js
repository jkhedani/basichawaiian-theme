 ...
 *  Mark scene viewed
 */
function markSceneComplete(postID) {
...
// If user visits the home page, check to see if they have visited before.
	...
	if ( $('body').hasClass('home') ) {
		var postID = $('#content').data('post-id'); // Get the post ID
	...
	if ( $('body').hasClass('single-units') ) {
		...
		var postID = $('article').data('post-id'); // Get the post ID
		...
		var sceneViewedHandler = function() {
			markSceneComplete( postID );
			jQuery('body').find('#content').show();
		}
		$(document).on( 'click','a.end-scene', sceneViewedHandler );
